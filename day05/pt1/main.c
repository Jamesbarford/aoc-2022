...
char *solve(rFile *rf) {
    ...
    lNode *ln = instruction_set->root;
    while (ln) {
        unsigned long count = ln->value >> 16 & 0xFF;
        unsigned long from = (ln->value >> 8 & 0xFF);
        unsigned long to = (ln->value & 0xFF);
