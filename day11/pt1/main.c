...
int monkeyDoOp(monkey *m, int value) {
    switch (m->op) {
        case ADD:
...
int __qsidx(int *arr, int high, int low) {
    int pivot = arr[high];
    int idx = low - 1;
...
void solve(monkey **monkies, int count, int rounds) {
    int touches[MONKEY_COUNT] = {0};
    monkey *m = NULL;
    ...
    for (int r = 0; r < rounds; ++r) {
        for (int i = 0; i < count; ++i) {
            ...
            for (lNode *ln = l->head; ln != NULL && l->size; ln = ln->next) {
                touches[m->id]++;
                ln->value = monkeyDoOp(m, ln->value);
                ln->value /= 3;

                printf("val: %d div: %d -> %d\n", ln->value, m->divisor,
                        ln->value % m->divisor);
                if (ln->value % m->divisor == 0) {
                    listAppend(monkies[m->true_monkey]->items, ln->value);
                } else {
                    listAppend(monkies[m->false_monkey]->items, ln->value);
                }
            }
            listClear(l);
        }
    ...
    printf("count: %d\n", touches[count-1] * touches[count-2]);
}
