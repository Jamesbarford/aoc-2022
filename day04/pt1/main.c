...
int hashsetFullIntersection(hashset *h1, hashset *h2) {
    int retval = 1;
    hashset *i1 = hashsetIntersection(h1,h2);
    hashset *i2 = hashsetIntersection(h2,h1);

    for (int i = 0; i < SLAB_SIZE && retval == 1; ++i) {
        hNode *hn = h1->slab[i];
        while (hn) {
            if (!hashsetHas(i2,hn->value) && !hashsetHas(i1,hn->value)) {
                retval = 0;
                break;
            }
            hn = hn->next;
    ...
    while (*ptr) {
        switch (*ptr) {
            ...
            case '\n':
                ...
                isstart = 1;
                acc += hashsetFullIntersection(h1,h2);
                hashsetClear(h1);
                hashsetClear(h2);
                break;
